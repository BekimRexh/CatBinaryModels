...
####DONT RUN THIS####
#### very similar to iteration twelve but it looks better in the f1 scores so it has good potential #####
#### BEST MODEL SO FAR IN TERMS OF F1 SCORES .91 AMD .93  
...
model.compile(
    optimizer='adam',
    loss=tf.keras.losses.BinaryCrossentropy(label_smoothing=0.15),  # Added label smoothing
    metrics=['accuracy']
)
...
model.save('binaryCnn15-model.h5')
